...
Frequency <- function(x, nb) {
  ...
  for(i in 1:length(x)) {
    ...
    for(j in 1:nbBitsToConsider) {
      ...
      bit0or1 <- seq32Bits[32+1 - j] # see bit order with cat(binary(12), '\n')
...
```
average pValeur VonNeumann :  1.5e-07 
average pValeur RANDU :  0.12 
average pValeur StandardMinimal :  7.7e-05 
average pValeur MersenneTwister :  7e-06 
...
cat('average pValeur MersenneTwister : ', computeAvgPValeur(MersenneTwister, 1000, 100), '\n')
```
average pValeur VonNeumann :  1.2e-07 
average pValeur RANDU :  0.054 
average pValeur StandardMinimal :  1.8e-06 
average pValeur MersenneTwister :  8e-06 
...
average pValeur (runs) RANDU :  0.024 
average pValeur (runs) StandardMinimal :  0.042 
average pValeur (runs) MersenneTwister :  0.032 
...
computeAvgPValeurOrder <- function(generator, lengthSeq, repetition, maxSeed=100000000) {
  ...
  sumPValeur <- 0
  for (i in 1:repetition) {
    u <- as.numeric(generator(lengthSeq, seeds[i])) # TODO: understand why as.numeric, does transposition (row to column)
