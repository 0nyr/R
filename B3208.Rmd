...
* terme_gauche = E(N) = alpha/(1 - alpha)

* terme_droite = lambda*E(W) = somme des temps passés dans la file pour chaque client divisé par le nombre de clients
...
for(i in c(6, 10, 11, 15)) {
  ...
  # calcul terme de gauche
  alpha = lambda / mu
  terme_gauche <- alpha / (1 - alpha)
  
  # calcul terme de droite
  nb_clients <- length(file$arrivees)
  terme_droite <- SommeTempsPasseFile(file$arrivees, file$departs) / nb_clients
  
  cat("Pour lambda =", i, ": moyenne des personnes =", moy_pers, ", attente moyenne =", attente_moy, "\n", "Terme gauche =", terme_gauche, "Terme droite =", terme_droite)
}
```

TODO : constatation sur l'égalité terme_droite, terme_gauche.
